...
object hlist {
  ...
  object NatTRel {
    ...
    implicit def hnilNatTRel1[F1[_], F2[_]]: shapeless.ops.hlist.NatTRel[shapeless.HNil,F1,shapeless.HNil,F2] = new NatTRel[HNil, F1, HNil, F2] {
    ...
    implicit def hlistNatTRel1[H, F1[_], F2[_], T1 <: HList, T2 <: HList](implicit nt : NatTRel[T1, F1, T2, F2]): shapeless.ops.hlist.NatTRel[F1[H] :: T1,F1,F2[H] :: T2,F2] =
      new NatTRel[F1[H] :: T1, F1, F2[H] :: T2, F2] {
        def map(f: F1 ~> F2, fa: F1[H] :: T1): F2[H] :: T2 = f(fa.head) :: nt.map(f, fa.tail)
      }

    implicit def hlistNatTRel2[H, F2[_], T1 <: HList, T2 <: HList](implicit nt : NatTRel[T1, Id, T2, F2]): shapeless.ops.hlist.NatTRel[H :: T1,shapeless.Id,F2[H] :: T2,F2] =
      new NatTRel[H :: T1, Id, F2[H] :: T2, F2] {
        def map(f: Id ~> F2, fa: H :: T1): F2[H] :: T2 = f(fa.head) :: nt.map(f, fa.tail)
      }

    implicit def hlistNatTRel3[H, F1[_], T1 <: HList, T2 <: HList](implicit nt : NatTRel[T1, F1, T2, Id]): shapeless.ops.hlist.NatTRel[F1[H] :: T1,F1,H :: T2,shapeless.Id] =
      new NatTRel[F1[H] :: T1, F1, H :: T2, Id] {
        def map(f: F1 ~> Id, fa: F1[H] :: T1): H :: T2 = f(fa.head) :: nt.map(f, fa.tail)
      }

    implicit def hlistNatTRel4[H1, F1[_], T1 <: HList, H2, T2 <: HList](implicit nt : NatTRel[T1, F1, T2, Const[H2]#λ]): shapeless.ops.hlist.NatTRel[F1[H1] :: T1,F1,H2 :: T2, Const[H2]#λ] =
      new NatTRel[F1[H1] :: T1, F1, H2 :: T2, Const[H2]#λ] {
        def map(f: F1 ~> Const[H2]#λ, fa: F1[H1] :: T1): H2 :: T2 = f(fa.head) :: nt.map(f, fa.tail)
      }

    implicit def hlistNatTRel5[H1, T1 <: HList, H2, T2 <: HList](implicit nt : NatTRel[T1, Id, T2, Const[H2]#λ]): shapeless.ops.hlist.NatTRel[H1 :: T1,shapeless.Id,H2 :: T2, Const[H2]#λ] =
  ...
  object HKernelAux {
    ...
    implicit def mkHNilHKernel: shapeless.ops.hlist.HKernelAux[shapeless.HNil]{type Out = shapeless.ops.hlist.HNilHKernel; def apply(): shapeless.ops.hlist.HNilHKernel.type} = new HKernelAux[HNil] {
      type Out = HNilHKernel
      def apply() = HNilHKernel
    }

    implicit def mkHListHKernel[H, T <: HList, CtOut <: HKernel](implicit ct: HKernelAux.Aux[T, CtOut]): shapeless.ops.hlist.HKernelAux[H :: T]{type Out = shapeless.ops.hlist.HConsHKernel[H,CtOut]} = new HKernelAux[H :: T] {
 ...
 trait Grouper[L <: HList, N <: Nat, Step <: Nat] extends DepFn1[L] with Serializable {
  ...
  object RightScanner {
    ...
    implicit def hlistRightScanner0[H, H0, T <: HList, P <: Poly, C2Result](implicit ev: Case2.Aux[P, H0, H, C2Result]): shapeless.ops.hlist.RightScanner.RightScanner0[H :: T,H0,P]{type Out = C2Result :: H :: T} =
  ...
  object Patcher {
    ...
    implicit def hlistPatch2[M <: Nat, L <: HList, In <: HList, OutL <: HList, OutP <: HList]
      (implicit drop: Drop.Aux[L, M, OutL], prepend: Prepend.Aux[In, OutL, OutP]): shapeless.ops.hlist.Patcher[shapeless._0,M,L,In]{type Out = OutP} =
  ...
  */
  ...
  trait LowPriorityCombinations {
    implicit def combinationHNil[N <: Nat]: shapeless.ops.hlist.Combinations[N,shapeless.HNil]{type Out = shapeless.HNil} =
