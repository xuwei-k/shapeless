...
object traversable {
  ...
  object FromTraversable {
    ...
    implicit def hnilFromTraversable[T]: shapeless.ops.traversable.FromTraversable[shapeless.HNil] = new FromTraversable[HNil] {
      def apply(l : Iterable[_]) =
        if(l.isEmpty) Some(HNil) else None 
    }
    
    implicit def hlistFromTraversable[OutH, OutT <: HList]
      (implicit flt : FromTraversable[OutT], oc : Typeable[OutH]): shapeless.ops.traversable.FromTraversable[OutH :: OutT] = new FromTraversable[OutH :: OutT] {
