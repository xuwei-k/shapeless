...
object sized {
  implicit def genTraversableSizedConv[Repr](cc : Repr)
    (implicit iil: IsRegularIterable[Repr], ev : AdditiveCollection[Repr]): shapeless.syntax.SizedConv[Repr] =
      new SizedConv[Repr](cc)
  
  implicit def stringSizedConv(s : String): shapeless.syntax.SizedConv[String] = new SizedConv[String](s)
}
