 ...
 */
object traversable {
  implicit def traversableOps[T](t : T)(implicit ev: T => Iterable[_]): shapeless.syntax.std.TraversableOps[T] = new TraversableOps(t)
  implicit def traversableOps2[CC[T] <: Iterable[T], A](as: CC[A]): shapeless.syntax.std.TraversableOps2[CC,A] = new TraversableOps2(as)
}
