...
object function {
  ...
  implicit def fnHListOps[F, T <: HList, R](t: F)(implicit fnHLister: FnToProduct.Aux[F, T => R]): shapeless.syntax.std.FnHListOps[T => R] = new FnHListOps[T => R] {
    def toProduct = fnHLister(t)
  }


  implicit def fnUnHListOps[F](t : F)(implicit fnUnHLister : FnFromProduct[F]): shapeless.syntax.std.FnUnHListOps[fnUnHLister.Out] = new FnUnHListOps[fnUnHLister.Out] {
