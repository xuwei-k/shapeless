...
object PolyDefns extends Cases {
  ...
  object Compose {
    implicit def composeCase[C, F <: Poly, G <: Poly, T, U, V]
      (implicit unpack: Unpack2[C, Compose, F, G], cG : Case1.Aux[G, T, U], cF : Case1.Aux[F, U, V]): shapeless.PolyDefns.Case[C,T :: shapeless.HNil]{type Result = V} = new Case[C, T :: HNil] {
      type Result = V
  ...
  trait LowPriorityLiftFunction1 extends Poly1 {
    implicit def default[T]: LowPriorityLiftFunction1.this.Case.Aux[T,shapeless.HNil] = at[T](_ => HNil : HNil)
  }
  ...
  class >->[T, R](f : T => R) extends LowPriorityLiftFunction1 {
    implicit def subT[U <: T]: >->.this.Case.Aux[U,R :: shapeless.HNil] = at[U](f(_) :: HNil)
  }

  trait LowPriorityLiftU extends Poly {
    implicit def default[L <: HList]: LowPriorityLiftU.this.ProductCase[L]{type Result = shapeless.HNil; val value: L => shapeless.HNil.type} = new ProductCase[L] {
      type Result = HNil
      val value = (l : L) => HNil
    }
  }
  ...
  class LiftU[P <: Poly](p : P)  extends LowPriorityLiftU {
    implicit def defined[L <: HList](implicit caseT : Case[P, L]): LiftU.this.ProductCase[L]{type Result = caseT.Result :: shapeless.HNil} = new ProductCase[L] {
